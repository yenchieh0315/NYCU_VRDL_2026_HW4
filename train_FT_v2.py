...
class RestorationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # ==========================================
        # (3) 新增：CutBlur 數據增強 (機率 0.5)
        # ==========================================
        if random.random() < 0.5:
            _, h, w = img_deg.shape
            
            # 隨機決定覆蓋區塊的大小 (寬高皆為原圖的 1/4 ~ 1/2)
            ...
            # 將乾淨影像的該區塊，覆蓋到退化影像的對應位置上
            # 目的：強迫模型學會「不要破壞原本就乾淨的區域」
            img_deg[:, cy:cy+ch, cx:cx+cw] = img_clean[:, cy:cy+ch, cx:cx+cw].clone()
...
class EdgeLoss(nn.Module):
    def __init__(self):
        super().__init__()
        # 定義 Sobel 邊緣偵測的 Kernel (X 方向與 Y 方向)
        k_x = torch.tensor([[-1, 0, 1], [-2, 0, 2], [-1, 0, 1]], dtype=torch.float32).unsqueeze(0).unsqueeze(0)
        k_y = torch.tensor([[-1, -2, -1], [0, 0, 0], [1, 2, 1]], dtype=torch.float32).unsqueeze(0).unsqueeze(0)
        
        # 擴張為 3 通道 (RGB)
        self.weight_x = nn.Parameter(k_x.repeat(3, 1, 1, 1), requires_grad=False)
        self.weight_y = nn.Parameter(k_y.repeat(3, 1, 1, 1), requires_grad=False)

    def forward(self, pred, target):
        # 使用 reflection padding 避免邊界效應
        pred_pad = F.pad(pred, (1, 1, 1, 1), mode='reflect')
        target_pad = F.pad(target, (1, 1, 1, 1), mode='reflect')
        
        # 分別對預測圖和乾淨圖進行卷積，擷取邊緣特徵
        pred_edge_x = F.conv2d(pred_pad, self.weight_x.to(pred.device), groups=3)
        pred_edge_y = F.conv2d(pred_pad, self.weight_y.to(pred.device), groups=3)
        target_edge_x = F.conv2d(target_pad, self.weight_x.to(target.device), groups=3)
        target_edge_y = F.conv2d(target_pad, self.weight_y.to(target.device), groups=3)
        
        # 計算梯度的強度 (Magnitude)
        edge_pred = torch.sqrt(pred_edge_x**2 + pred_edge_y**2 + 1e-6)
        edge_target = torch.sqrt(target_edge_x**2 + target_edge_y**2 + 1e-6)
...
def main():
    ...
    previous_weight = './V3_fine_tuning/v3_fine_tuning_pth/v3_epoch50_fine-tuning.pth'
    ...
    for epoch in range(EPOCHS):
        ...
        for degraded_img, clean_img in pbar:
            ...
            with torch.autocast('cuda', dtype=torch.bfloat16):
                ...
                loss_edge = criterion_edge(restored_img, clean_img)
                loss = loss_charb + 0.05 * loss_fft + 0.1 * loss_edge
        ...
        is_last_5 = (epoch + 1) > (EPOCHS - 5)
        if is_checkpoint_epoch or is_last_5:
            save_path = f'v3_epoch{epoch+1}_FT_v2.pth'

...
# 從你的 train_fine_tuning.py 匯入你設計好的模型架構
from train_fine_tuning import PromptIR_Hybrid 
...
def average_weights(weight_paths, device):
    """
    讀取多個權重檔，並將它們的參數進行平均 (SWA)。
    """
    print(f"正在執行 SWA，準備平均 {len(weight_paths)} 個權重...")
    avg_state_dict = None
    ...
    for k in avg_state_dict.keys():
        if avg_state_dict[k].dtype.is_floating_point:
            avg_state_dict[k] = avg_state_dict[k] / len(weight_paths)
        else:
            # 針對非浮點數的參數 (如 num_batches_tracked) 採用 // 運算
            avg_state_dict[k] = avg_state_dict[k] // len(weight_paths)
...
def main():
    ...
    # 填入你要進行 SWA 平均的權重路徑清單 (請依據你實際存檔的路徑與檔名修改)
    base_path = './V3_FT_v2/v3_FT_v2_pth'
    swa_weight_paths = [
        os.path.join(base_path, 'v3_epoch96_FT_v2.pth'),
        os.path.join(base_path, 'v3_epoch97_FT_v2.pth'),
        os.path.join(base_path, 'v3_epoch98_FT_v2.pth'),
        os.path.join(base_path, 'v3_epoch99_FT_v2.pth'),
        os.path.join(base_path, 'v3_epoch100_FT_v2.pth')
    ]
    output_npz = 'pred.npz'
    ...
    avg_state_dict = average_weights(swa_weight_paths, device)
